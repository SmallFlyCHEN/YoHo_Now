...
public class CommunityFancyCoverFlowAdapter extends FancyCoverFlowAdapter{
    ...
    public View getCoverFlowItem(int position, View reusableView, ViewGroup parent) {
        ...
        if (reusableView == null){
            ...
            viewHolder.ivHeadIconOne = (CircleImageView) reusableView.findViewById(R.id.item_community_iv_headIcon_one);
            viewHolder.ivHeadIconTwo = (CircleImageView) reusableView.findViewById(R.id.item_community_iv_headIcon_two);
            viewHolder.tvForumName = (TextView) reusableView.findViewById(R.id.item_community_tv_forumName);
        ...
        if (position == 0){
            ...
            Picasso.with(context).load(fancyCoverFlowBean.getData().getForumInfo().get(position - 1).getForumPic())
                    .into(viewHolder.ivForumPic);
            Picasso.with(context).load(CommunityAdapter.getURLString(fancyCoverFlowBean.getData().getForumInfo().get(position - 1).getHotPost().getUser()
                    .getHeadIcon())).into(viewHolder.ivHeadIconOne);

            Picasso.with(context).load(CommunityAdapter.getURLString(fancyCoverFlowBean.getData().getForumInfo().get(position - 1).getNewPost().getUser()
            ...
            viewHolder.tvPostsNum.setText("帖子" + String.valueOf(fancyCoverFlowBean.getData().getForumInfo().get(position-1)
                    .getPostsNum()));
    ...
    private static class FcfViewHolder {
        ImageView ivForumPic;
        CircleImageView ivHeadIconOne, ivHeadIconTwo;
